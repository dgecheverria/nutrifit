(function ($) {
    ...
    function quick_product_variable() {
        ...
        $(document).on('click', btnSelector, function (e) {
            ...
            if ($product.hasClass('quick-shop-loaded')) {
                ...
                $('body').trigger('nutritix-quick-view-displayed');
                return;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // nutritixThemeModule.swatchesVariations();
            ...
            // Ajax action.
            xhr = $.ajax({
                url: wc_add_to_cart_params.wc_ajax_url.toString().replace('%%endpoint%%', 'nutritix_add_to_cart'),
                data: formData,
                type: 'POST',
                processData: false,
                contentType: false,
    ...
    function sendRequest(url) {
        ...
        xhr = $.ajax({
            ...
            url: url,
            beforeSend: function () {
                var $products = $('ul.nutritix-products');
                $products.addClass('preloader');
            },
            success: function (data) {
                let $html = $(data);
                $('#main ul.nutritix-products').replaceWith($html.find('#main ul.nutritix-products'));
                $('#main .woocommerce-pagination').replaceWith($html.find('#main .woocommerce-pagination'));
                window.history.pushState(null, null, url);
                xhr = false;
                $(document).trigger('nutritix-products-loaded')
            }
        });
    }

    $body.on('change', '.nutritix-products-per-page #per_page', function (e) {
        e.preventDefault();
        var url = this.value;
        sendRequest(url);
    });
