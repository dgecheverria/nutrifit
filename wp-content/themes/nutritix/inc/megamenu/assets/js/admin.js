( function($) {
	var Nutritix_Modal_Menu_Item = Backbone.View.extend({

		initialize: function( data ) {
			this.model = new Backbone.Model( data );
			this.template = '<span class="nutritix-edit-menu"><i class="fa fa-pencil"></i> Mega</span>';
		...
		_openModalHandler: function() {
			window.nutritix_menu_modal = new Nutritix_Modal( this.model );
		}

	});

	// Backbone Modal View
	var Nutritix_Modal = Backbone.View.extend({
		...
		initialize: function( model ) {
			...
			this.events = {
				...
				'click .nutritix-modal-overlay' : '_closeHandler',
		...
		render: function() {
			var that = this;
			this.template = $( '#tpl-nutritix-menu-item-modal' ).html();
			...
			data.action = 'nutritix_load_menu_data';
			data.nonce = nutritix_memgamnu_params.nonces.load_menu_data;
			...
			if ( this.model.get( 'is_loading' ) === true ) {
				$.ajax({
					type: 'POST',
					url: nutritix_memgamnu_params.ajaxurl,
					data: data,
					beforeSend: function() {
		...
		_submitEditMenuForm: function( e ) {
			...
			data.push({
				name: 'action',
				value: 'nutritix_update_menu_item_data'
			});
			...
			$.ajax({
				...
				data: data,
				beforeSend: function() {
					form.find( '.nutritix-modal-footer button' ).attr( 'disabled', true );
					form.find( 'button[type="submit"]' ).prepend( '<i class="fa fa-spin fa-spinner"></i> ' );
				}
			}).always( function() {
				form.find( '.nutritix-modal-footer button' ).attr( 'disabled', false );
			} ).done( function( res ) {
				if ( res.status === true ) {
					that.model.destroy();
				}
	...
	$( document ).on( 'mouseenter', '#menu-to-edit .menu-item-handle', function(e) {
		...
		menu_title.append( new Nutritix_Modal_Menu_Item({
			menu_id: menu_id,
			is_loading: true,
			istop:istop?1:0
		}).render().el );
		...
		var menu_item = $( this );
		var menu_title = menu_item.find( '.item-title' );
		menu_title.find( '.nutritix-edit-menu' ).remove();
	} );
} )(jQuery);
