 ...
 * @extends WC_Widget
 */
class Nutritix_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'nutritix');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'nutritix');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'nutritix'),
				'label' => esc_html__('Title', 'nutritix'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'nutritix'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'nutritix'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'nutritix'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'nutritix'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'nutritix'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'nutritix'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'nutritix'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Nutritix_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'nutritix');
			...
			echo '<ul class="product-brands" id="nutritix-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Nutritix_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'nutritix'),
		'singular_name'              => esc_html__('Brands', 'nutritix'),
		'menu_name'                  => esc_html__('Brands', 'nutritix'),
		'all_items'                  => esc_html__('All Brands', 'nutritix'),
		'parent_item'                => esc_html__('Parent Brand', 'nutritix'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'nutritix'),
		'new_item_name'              => esc_html__('New Brand Name', 'nutritix'),
		'add_new_item'               => esc_html__('Add New Brands', 'nutritix'),
		'edit_item'                  => esc_html__('Edit Brand', 'nutritix'),
		'update_item'                => esc_html__('Update Brand', 'nutritix'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'nutritix'),
		'search_items'               => esc_html__('Search Brands', 'nutritix'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'nutritix'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'nutritix'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
