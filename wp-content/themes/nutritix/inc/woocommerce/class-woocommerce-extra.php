 ...
 */
class Nutritix_Woocommerce_Extra {
    ...
    public function __construct() {
        ...
        add_action('woocommerce_product_options_sku', array($this, 'add_field_model'));
    ...
    public function add_deal_fields() {
        ...
        <p class="form-field _deal_quantity_field">
            <label for="_sale_quantity"><?php esc_html_e('Sale quantity', 'nutritix') ?></label>
            <?php echo wc_help_tip(__('Set this quantity will make the product to be a deal. The sale will end when this quantity is sold out.', 'nutritix')); ?>
            ...
            <?php
            if ($quantity > 0) {
                echo '<span class="deal-sold-counts"><strong>' . sprintf(_n('%s product is sold', '%s products are sold', max(1, $sales_counts), 'nutritix'), $sales_counts) . '</strong></span>';
            }
            ?>
        </p>
    ...
    public function add_field_model(){
        global $thepostid;
        $unit     = get_post_meta($thepostid, '_model', true);
        ?>
        <p class="form-field _model_field">
            <label for="_model"><?php esc_html_e('Model', 'nutritix') ?></label>
            <input type="text" class="short" name="_model" id="_model" value="<?php echo esc_attr($unit) ?>">
    ...
    public function save_product_data($post_id) {
        ...
        if(isset($_POST['_model'])){
            update_post_meta($post_id, '_model', wc_clean($_POST['_model']));
        }
        else {
    ...
    public function quantity_input_args($args, $product) {
        if (!nutritix_woocommerce_is_deal_product($product)) {
            return $args;
        }
...
new Nutritix_Woocommerce_Extra;
