<?php
/**
 * Nutritix WooCommerce Settings Class
 *
 * @package  nutritix
 * @since    2.4.3
 */
...
if (!class_exists('Nutritix_WooCommerce_Settings')) :

    /**
     * The Nutritix WooCommerce Settings Class
     */
    class Nutritix_WooCommerce_Settings {
        ...
        public function settings_product_tabs($tabs) {
            ...
            $tabs['sizechart'] = array(
                'label'    => esc_html__('Nutritix settings', 'nutritix'),
                'target'   => 'nutritix_options',
                'class'    => array(),
                'priority' => 80,
            );
        ...
        public function settings_options_product_tab_content() {
            ...
            ?>
            <div id='nutritix_options' class='panel woocommerce_options_panel'>
                <div class='options_group'><?php
                    ...
                    $options[''] = esc_html__('Select size chart', 'nutritix');
                    ...
                    woocommerce_wp_select(array(
                        ...
                        'label'   => esc_html__('Size chart', 'nutritix'),
                        'options' => $options,
                        'value'   => $value,
                    ));

                    ?>
                    <p class="form-field form-field-wide wc-customer-custom"><?php echo esc_html__('Size chart will take templates name prefix is "SizeChart"','nutritix');?></p>

                    <?php
                    woocommerce_wp_text_input(array(
                        'id'    => '_video_select',
                        'label' => esc_html__('Url Video', 'nutritix'),
                    ));
                    woocommerce_wp_textarea_input(array(
                        'id'      => '_extra_info',
                        'label'   => esc_html__('Extra info', 'nutritix'),
                    ));
                    ?>
                </div>
        ...
        public function render_sizechart_button() {
            ...
            $chart_id = $this->check_chart($slug);
            if (!empty($slug) && $chart_id) {
                echo '<a href="#" class="sizechart-button">' . esc_html__('Size Guide', 'nutritix') . '</a>';
            }
        }
        ...
        public function render_sizechart_template() {
            ...
            $chart_id = $this->check_chart($slug);
            if (!empty($slug) && $chart_id) {
                echo '<div class="sizechart-popup"><a href="#" class="sizechart-close"><i class="nutritix-icon-times-circle"></i></a>' . Elementor\Plugin::instance()->frontend->get_builder_content($chart_id) . '</div><div class="sizechart-overlay"></div>';
            }
        }
    }

    return new Nutritix_WooCommerce_Settings();
