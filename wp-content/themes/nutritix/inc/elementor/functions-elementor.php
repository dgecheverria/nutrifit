<?php
if ( ! function_exists( 'nutritix_elementor_get_render_attribute_string' ) ) {
	function nutritix_elementor_get_render_attribute_string($element, $obj) {
		return $obj->get_render_attribute_string($element);
	}
}
if ( ! function_exists( 'nutritix_elementor_parse_text_editor' ) ) {
	function nutritix_elementor_parse_text_editor( $content, $obj ) {
...
if ( ! function_exists( 'nutritix_elementor_get_strftime' ) ) {
	function nutritix_elementor_get_strftime( $instance, $obj ) {
		$string = '';
...
if (!function_exists('nutritix_elementor_breakpoints')) {
    function nutritix_elementor_breakpoints() {

        $breakpoints = \Elementor\Plugin::$instance->breakpoints->get_breakpoints();
        $var ='';
        $check = nutritix_is_woocommerce_activated();
        foreach (array_reverse($breakpoints) as $breakpoint) {
            if ($breakpoint->is_enabled()) {
                ...
                for ($i = 1; $i <= 8; $i++) {
                    ...
                    $var .= 'body.theme-nutritix [data-elementor-columns-'.$device_name.'="'.$i.'"] .column-item{flex: 0 0 '.$ratio.'%; max-width: '.$ratio.'%;}';
                    if($check){
