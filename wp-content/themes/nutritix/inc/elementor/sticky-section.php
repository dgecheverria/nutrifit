...
class Nutritix_Sticky_Module {
	...
	public function scripts(){
        ...
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/jquery.sticky.js', ['jquery'], $nutritix_version, true);
        wp_enqueue_script('nutritix-elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], $nutritix_version, true);
    }
    ...
    public function register_controls( Element_Base $element ) {
        $element->add_control(
            'sticky',
            [
                'label' => esc_html__( 'Sticky', 'nutritix' ),
                'type' => Controls_Manager::SELECT,
                'options' => [
                    '' => esc_html__( 'None', 'nutritix' ),
                    'top' => esc_html__( 'Top', 'nutritix' ),
                    'bottom' => esc_html__( 'Bottom', 'nutritix' ),
                ],
                'separator' => 'before',
                'render_type' => 'none',
        ...
        foreach ( $active_devices as $device ) {
            $label = 'desktop' === $device ? esc_html__( 'Desktop', 'nutritix' ) : $active_breakpoint_instances[ $device ]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label' => esc_html__( 'Sticky On', 'nutritix' ),
        ...
        $element->add_responsive_control(
            'sticky_offset',
            [
                'label' => esc_html__( 'Offset', 'nutritix' ),
        ...
        $element->add_responsive_control(
            ...
            [
                'label' => esc_html__( 'Effects Offset', 'nutritix' ),
                'type' => Controls_Manager::NUMBER,
                'default' => 0,
                'min' => 0,
                'max' => 1000,
        ...
        if ( $this->is_instance_of( $element, $types ) ) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label' => esc_html__( 'Stay In Column', 'nutritix' ),
